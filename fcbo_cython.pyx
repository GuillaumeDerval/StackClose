...
cdef inline void init_stack_element(stack_element* elem, int n_row_long, int n_col_long, int n_col):
...
cdef inline long* get_set_from_array(long* a, int line_length, int idx):
    return &(a[idx*line_length])
...
#@cython.boundscheck(False)  # Deactivate bounds checking
#@cython.wraparound(False)   # Deactivate negative indexing.
cdef __fcbo(double[:,:] matrix, process):
    ...
    for i in range(n_cols):
        init_stack_element(&stack[i], n_row_long, n_col_long, n_cols)
